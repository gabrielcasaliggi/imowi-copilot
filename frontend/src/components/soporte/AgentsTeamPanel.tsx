...
import { GlassCard, KpiCard, SidebarSection } from "@/components/ui/GlassCard";
...
export function AgentsTeamPanel() {
  ...
  const canManage = can("users.manage_agents");
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (canStats) {
        ...
        setAbiertos(data.tickets_abiertos || 0);
      } else {
  ...
  if (!canStats && !canManage) return null;
  ...
  return (
    <div className="space-y-4">
      <SidebarSection title="Equipo de agentes">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
          <KpiCard label="Agentes" value={agentes.length} tone="cyan" />
          <KpiCard label="Cola abierta" value={abiertos} tone="amber" />
          <KpiCard
            label="Disponibles"
            value={agentes.filter((a) => a.disponibilidad === "disponible" && a.activo !== false).length}
            tone="emerald"
          />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <GlassCard title="Carga y disponibilidad" accent="cyan" variant="secondary">
            {loading ? (
                ...
                {agentes.map((a) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-slate-500 truncate text-[11px]">{a.email}</p>
                    ...
                    <div className="text-right shrink-0 space-y-1">
                      <p className="font-mono text-slate-300">
                        {a.tickets_abiertos} abiertos
                      </p>
                      <p className="text-[11px] text-slate-500">
                        {a.tickets_cerrados} cerrados
                      </p>
                      {canManage && (
                        <button
                          type="button"
