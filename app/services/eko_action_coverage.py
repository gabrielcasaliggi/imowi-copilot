...
def _base_rows() -> list[ActionCoverage]:
    return [
        ...
        ActionCoverage(
            action="show_invoice",
            status="EXECUTED_BY_RUNTIME",
            legacy_call_site="eko_journeys._advance_billing_invoice (else)",
            runtime_call_site="eko_journeys._advance_billing_invoice → dispatch",
            feature_gate="ACTION_RUNTIME_ENABLED + ACTIONS",
            executor=True,
            confirmation="NOT_REQUIRED",
            idempotency="SAFE",
            notes="FC header only; ownership=client_number; no due/period/PDF",
        ),
        ActionCoverage(
            action="service_list",
            status="EXECUTED_BY_RUNTIME",
            legacy_call_site="eko_journeys._advance_service_catalog (else)",
            runtime_call_site="eko_journeys._advance_service_catalog → dispatch",
            feature_gate="ACTION_RUNTIME_ENABLED + ACTIONS",
            executor=True,
            confirmation="NOT_REQUIRED",
            idempotency="SAFE",
            notes="Commercial catalog via evaluar_servicios_portal; no selection/tech",
        ),
        ActionCoverage(
            action="installation_status",
            status="EXECUTED_BY_RUNTIME",
            legacy_call_site="eko_journeys._advance_installation_status (honest msg)",
            runtime_call_site="eko_journeys._advance_installation_status → dispatch",
            feature_gate="ACTION_RUNTIME_ENABLED + ACTIONS",
            executor=True,
            confirmation="NOT_REQUIRED",
            idempotency="SAFE",
            notes="2.2D UNAVAILABLE — no agenda/órdenes; always source_unavailable",
        ),
        ActionCoverage(
            action="show_ticket",
            status="EXECUTED_BY_RUNTIME",
